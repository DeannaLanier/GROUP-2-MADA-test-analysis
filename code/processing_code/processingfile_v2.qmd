...
*Forehead Height (seems to be in inches)
* Sex
*hair color
...
Variable `Forehead Height` values also seem reasonable in inches.
...
Well proceed here by removing that individual with that NA entry. Since this keeps an empty category for Sex, I'm also using droplevels() to get rid of it. Now checking `Hair color` variable, it is character, so it seems correct. However, let's remove the space in the column tile for both `Forehead height`and `Hair color`
